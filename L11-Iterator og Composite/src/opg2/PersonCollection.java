...
public class PersonCollection implements Iterable<Person>{
    ...
    private int getÆndringsKode() {
        return ændringsKode;
    }

    public Iterator<Person> iterator() {
        return new PerseonIterator(persons, ændringsKode);
    }


    private class PerseonIterator implements Iterator<Person> {
        Person[] persons;
        int position = 0;
        int ændringsKode;

        public PerseonIterator(Person[] persons, int ændringsKode) {
            this.persons = persons;
            this.ændringsKode = ændringsKode;
        }

        @Override
        public boolean hasNext() {
            if(ændringsKode != getÆndringsKode())
